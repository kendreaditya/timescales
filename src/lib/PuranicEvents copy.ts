...
class Timeline {
  ...
  init() {
    ...
    this.visibleEvents = this.data
      .filter((event) => event.time <= this.endTime && event.time >= this.startTime)
  ...
  renderEvents() {
    ...
    this.visibleEvents.forEach((event, index) => {
      const idealPosition = this.timeToPixel(event.time);
      ...
      this.createConnectingLine(idealPosition, adjustedPosition);
